...
use lrs_v2::lrs::sign_no_opt;
use lrs_v2::lrs::verify_no_opt;
...
fn lrs_signature(n_iters: usize, log2_low: usize, log2_high: usize) {
    ...
    for ring_size_max_log in log2_low..=log2_high {
        ...
        for _iter in 0..n_iters {
            ...
            let signature = sign_no_opt::sign_no_opt::<ark_bn254::Bn254>(&lrs_pvkey, &circuit, &ring, msg, &mut sign_time);

            let mut verify_time = VerifyTime::new();
            let result =
                verify_no_opt::verify_no_opt::<ark_bn254::Bn254>(&lrs_pvkey, &ring, msg, &signature, &mut verify_time);
